#!/usr/bin/env python3
"""
model_prefactor_variant.py
===========================

*** ALTERNATIVE FUNCTIONAL FORM: PRE-EXPONENTIAL-FACTOR VARIANT ***
This is a variant of nplin_cnt_model.py (kept as a separate, independently
rerunnable copy -- the original is not modified). Instead of the light field
lowering the nucleation barrier dG* (the original mechanism), this variant
has the light field boost the CNT kinetic prefactor A directly:

    J = [A * (1 + kappa_boost)] * exp(-dG*_0 / kT)      (dG*_0 = UNMODIFIED barrier)

Physical justification (see TASK 1 in the request this was built for): A
represents the attachment rate of molecules to a forming cluster. A light
field could plausibly enhance this attachment rate directly (e.g. via
increased local molecular mobility, or a trapping/dielectrophoretic effect
drawing molecules toward the nucleation site) rather than lowering the
energy barrier itself.

IMPORTANT, non-obvious consequence of this functional form, discovered while
building this script (see run_calibrate_glycine() for the full treatment):
at a FIXED laser intensity (which is what the real glycine calibration data
uses -- one power density, not a fluence sweep), the ratio J(with)/J(no
field) = 1 + kappa_boost is a CONSTANT, independent of S. That means kappa
is fundamentally NOT IDENTIFIABLE from this dataset in this functional form:
any kappa can be exactly compensated by rescaling A, so a joint 2-parameter
fit to only 3 points at one intensity has a perfectly flat (degenerate)
direction in parameter space. This script detects and reports that directly
instead of pretending a joint fit produces a meaningful unique kappa.
...
*** PRE-EXPONENTIAL-FACTOR VARIANT (this file): *** instead of adding to the
bulk (volume) free-energy term / ln(S) (the original mechanism, still shown
below for reference), this variant instead boosts the kinetic prefactor A:

    ln(S_eff) = ln(S)                                    [UNCHANGED -- barrier
                                                           is NOT modified here]
    boost     = kappa * u_field * v_m / kT               [same dimensionless
                                                           quantity as the
                                                           original's ln(S)
                                                           shift, repurposed]
    A_eff     = A * (1 + boost)
    J         = A_eff * exp(-dG*_0 / kT)                 [dG*_0 = baseline,
                                                           unmodified barrier]

dG*, r* are therefore IDENTICAL to plain CNT in this variant -- only the
prefactor changes. See run_calibrate_glycine() for an important consequence
of this choice: at fixed laser intensity, J(with)/J(no field) = 1 + boost is
a CONSTANT independent of S, which makes kappa non-identifiable from a
single-intensity calibration dataset (see the module docstring above).
...
Requires: numpy, scipy, matplotlib (no other dependencies).
    ...
    python model_prefactor_variant.py demo
    ...
    python model_prefactor_variant.py explore
    ...
    python model_prefactor_variant.py calibrate --data yourdata.csv
    ...
    python model_prefactor_variant.py montecarlo
    ...
    python model_prefactor_variant.py litcheck
...
def prefactor_boost(u_field, v_m, T):
    """PRE-EXPONENTIAL-FACTOR VARIANT: dimensionless fractional boost to the CNT kinetic
    prefactor A, reusing the same u_field*v_m/kT quantity the original script used as a
    ln(S) shift -- here it multiplies A instead. Can be negative (fractional reduction) if
    eps_c < eps_s, in principle down to -1 (A_eff = 0); very negative values are clipped to
    -1 to avoid an unphysical negative rate."""
    boost = u_field * v_m / (K_B * T)
    return np.maximum(boost, -1.0)


def effective_ln_S(S, u_field, v_m, T):
    """PRE-EXPONENTIAL-FACTOR VARIANT: the barrier/ln(S) is NOT modified by the field in
    this variant -- kept only for interface compatibility with functions that expect it."""
    return np.log(S)


def delta_g_star_field(S, T, gamma, v_m, u_field):
    """PRE-EXPONENTIAL-FACTOR VARIANT: the barrier itself is unmodified by the field --
    this returns the same value as plain delta_g_star(). All of the field's effect in this
    variant lives in nucleation_rate_field()'s prefactor boost, not here."""
    return delta_g_star(S, T, gamma, v_m)


def critical_radius_field(S, T, gamma, v_m, u_field):
    """PRE-EXPONENTIAL-FACTOR VARIANT: critical radius is unmodified by the field (the
    barrier itself doesn't change in this variant -- see delta_g_star_field())."""
    return critical_radius(S, T, gamma, v_m)


def nucleation_rate_field(S, T, gamma, v_m, A, u_field):
    """CNT nucleation rate including the light-field term. PRE-EXPONENTIAL-FACTOR VARIANT:
    the field boosts the prefactor A -> A*(1+boost) instead of lowering the barrier; the
    barrier/exponential term is the same as plain nucleation_rate()."""
    boost = prefactor_boost(u_field, v_m, T)
    A_eff = A * (1.0 + boost)
    dG0 = delta_g_star(S, T, gamma, v_m)
    return A_eff * np.exp(-dG0 / (K_B * T))
...
def rate_ratio_vs_kappa(S, kappa, material: MaterialParams, I_peak, n_solvent, n_solid):
    """J(with field)/J(no field) at a given S and kappa. Computed as (prefactor ratio) x
    (barrier ratio) SEPARATELY, each numerically safe on its own, rather than as a raw
    J1/J0 division -- dividing raw J0/J1 exponentials risks a 0/0 NaN near the nucleation
    threshold where J0 underflows to exactly zero in floating point (this bit us during
    development; fixed here). PRE-EXPONENTIAL-FACTOR VARIANT: the barrier ratio is exactly
    1 (dG1==dG0 by construction, see delta_g_star_field()), so the entire ratio comes from
    the prefactor boost -- and notably, at fixed I_peak, this ratio is the SAME for every S."""
    ...
    boost = prefactor_boost(u_field, material.v_m, material.T)
    with np.errstate(over="ignore"):
        barrier_ratio = np.exp(-(dG1 - dG0) / (K_B * material.T))
    return (1.0 + boost) * barrier_ratio
...
def run_calibrate_glycine():
    ...
    print("*** PRE-EXPONENTIAL-FACTOR VARIANT: light field boosts A, not the barrier ***")
    ...
    material = MaterialParams(name="glycine, prefactor variant (aqueous, Javid et al. 2016 conditions)",
    ...
    # --- WHY THIS VARIANT'S kappa CANNOT BE FIT THE SAME WAY: IT IS NOT IDENTIFIABLE ---
    # In the barrier-modifying original, kappa affects the SHAPE of J(S) (because it enters
    # inside the (ln S)^-2 barrier), so fitting kappa against 3 points at different S is a
    # real (if steep/ill-conditioned) shape-fitting problem. In THIS variant, kappa only
    # rescales the prefactor: A_eff = A*(1+boost). Since boost depends on I_peak (fixed
    # across all 3 real data points -- one power density, not a fluence sweep) and NOT on S,
    # A_eff is a CONSTANT multiplier applied equally to every S. That means:
    #     model(S; kappa, A) = A*(1+boost(kappa)) * exp(-dG*_0(S)/kT) = A_eff * exp(-dG*_0(S)/kT)
    # is functionally IDENTICAL to plain CNT with a rescaled prefactor A_eff -- kappa and A
    # are perfectly degenerate (any kappa can be exactly compensated by adjusting A to keep
    # A_eff fixed). A joint 2-parameter curve_fit to 3 points at ONE intensity cannot
    # identify kappa separately from A here; it will silently converge to whatever the
    # initial guess/bounds nudge it toward, with a huge or meaningless uncertainty. Rather
    # than report a fake-precise kappa from an unidentifiable fit, this variant instead:
    #   1) fits log(A) ALONE with kappa FIXED at 0 (plain CNT, 1 free parameter),
    #   2) fits log(A) ALONE with kappa FIXED at the plausibility-bound edge kappa_max,
    #   3) shows that (1) and (2) give essentially the SAME fit quality -- proving kappa
    #      contributes nothing identifiable here, rather than asserting it by argument alone.
    print("\n--- Why this variant's kappa can't be fit like the original: it is NOT IDENTIFIABLE ---")
    print("At fixed laser intensity (this dataset uses one power density, not a fluence sweep),")
    print("A_eff = A*(1+boost(kappa)) is just a CONSTANT rescaling of A -- kappa and A are")
    print("perfectly degenerate here (unlike the barrier-modifying original, where kappa affects")
    print("the SHAPE of J(S), not just its overall scale). A joint (kappa, log A) fit to 3 points")
    print("at one intensity cannot separate them. Demonstrating this directly instead of asserting it:")

    def model_kappa0(S_arr, log_A):
        return model(S_arr, 0.0, log_A)

    popt0, pcov0 = curve_fit(model_kappa0, S_data, A_data, p0=[20.0], sigma=A_sigma,
                              absolute_sigma=True, bounds=([0.0], [40.0]), maxfev=20000)
    logA_k0 = popt0[0]
    preds_k0 = model_kappa0(S_data, logA_k0)
    ss_tot = np.sum((A_data - A_data.mean())**2)
    r2_k0 = 1 - np.sum((A_data - preds_k0)**2) / ss_tot
    ...
    def model_kappa_max(S_arr, log_A):
        return model(S_arr, kappa_max, log_A)

    popt_max, pcov_max = curve_fit(model_kappa_max, S_data, A_data, p0=[20.0], sigma=A_sigma,
                                    absolute_sigma=True, bounds=([0.0], [40.0]), maxfev=20000)
    logA_kmax = popt_max[0]
    preds_kmax = model_kappa_max(S_data, logA_kmax)
    r2_kmax = 1 - np.sum((A_data - preds_kmax)**2) / ss_tot

    print(f"\nkappa=0        (no light effect):  log10(A)={logA_k0:.4g}, R^2={r2_k0:.4f}")
    print(f"kappa=kappa_max={kappa_max:.4g} (ratio={MAX_PLAUSIBLE_NPLIN_RATIO:g}x, the most the plausibility")
    print(f"               bound permits): log10(A)={logA_kmax:.4g}, R^2={r2_kmax:.4f}")
    print(f"\nR^2 changed by only {abs(r2_kmax - r2_k0):.4f} between kappa=0 and the maximum kappa the")
    print(f"plausibility bound allows -- CONFIRMED: kappa is not identifiable from this fixed-")
    print(f"intensity dataset in this functional form. The data give NO reason to prefer any")
    print(f"kappa>0 over kappa=0; both are equally (un)supported once A is refit to compensate.")

    # --- DEMONSTRATION 1: run the SAME kind of joint (kappa, log A) curve_fit used by the
    # other two models, from several different initial guesses, all with the SAME bounds.
    # If kappa were identifiable, the optimizer would converge to (about) the same kappa
    # regardless of starting point. Showing that it does NOT is direct empirical evidence
    # of the degeneracy, on top of the theoretical argument and the boundary-case check above.
    print("\n--- Demonstration 1: the same joint 2-parameter fit as the other models, run from")
    print("    several different initial guesses (bounds kept identical each time) ---")
    joint_results = []
    for kappa0_guess in [1e-3, kappa_max * 0.1, kappa_max * 0.5, kappa_max * 0.9]:
        popt_j, _ = curve_fit(model, S_data, A_data, p0=[kappa0_guess, 20.0], sigma=A_sigma,
                               absolute_sigma=True, bounds=([0.0, 0.0], [kappa_max, 40.0]), maxfev=20000)
        kappa_j, logA_j = popt_j
        preds_j = model(S_data, kappa_j, logA_j)
        r2_j = 1 - np.sum((A_data - preds_j)**2) / ss_tot
        joint_results.append((kappa0_guess, kappa_j, logA_j, r2_j))
        print(f"  initial guess kappa0={kappa0_guess:12.4g}  ->  converged kappa={kappa_j:12.4g}, "
              f"log10(A)={logA_j:7.4g}, R^2={r2_j:.4f}")
    kappa_spread = max(k for _, k, _, _ in joint_results) - min(k for _, k, _, _ in joint_results)
    r2_spread = max(r for _, _, _, r in joint_results) - min(r for _, _, _, r in joint_results)
    print(f"\n  converged kappa spans a range of {kappa_spread:.4g} (from essentially 0 to near the")
    print(f"  search boundary) depending ONLY on the initial guess, while R^2 varies by just")
    print(f"  {r2_spread:.5f} -- exactly what non-identifiability looks like in practice: the")
    print(f"  optimizer happily reports a precise-looking kappa, but it is an artifact of where")
    print(f"  it started, not a measurement. This is why this variant does NOT report a single")
    print(f"  joint-fit kappa as 'the' answer the way the other two models do.")

    # --- DEMONSTRATION 2: Tikhonov/ridge-regularized fit -- among the whole degenerate family
    # of (kappa, A) pairs that fit equally well, add a small penalty on kappa itself and solve
    # for the MINIMUM-NORM kappa consistent with (approximately) the best achievable residual.
    # This is the standard, principled way to pick a single answer out of an underdetermined
    # problem, instead of an arbitrary curve_fit initial guess.
    from scipy.optimize import least_squares

    def residuals_ridge(params, lam):
        kappa, log_A = params
        model_pred = model(S_data, kappa, log_A)
        data_residuals = (model_pred - A_data) / A_sigma
        return np.concatenate([data_residuals, [lam * kappa]])

    print("\n--- Demonstration 2: Tikhonov-regularized (ridge) fit -- minimum-norm kappa among")
    print("    the degenerate family, for decreasing regularization strength lambda ---")
    for lam in [1.0, 1e-2, 1e-4]:
        res = least_squares(residuals_ridge, x0=[kappa_max * 0.5, 20.0], args=(lam,),
                             bounds=([0.0, 0.0], [kappa_max, 40.0]))
        kappa_ridge, logA_ridge = res.x
        preds_ridge = model(S_data, kappa_ridge, logA_ridge)
        r2_ridge = 1 - np.sum((A_data - preds_ridge)**2) / ss_tot
        print(f"  lambda={lam:8.4g}  ->  kappa={kappa_ridge:12.4g}, log10(A)={logA_ridge:7.4g}, "
              f"R^2={r2_ridge:.4f}")
    print("\n  As lambda shrinks (weaker penalty), the minimum-norm kappa converges to ~0 while")
    print("  R^2 stays the same as the unregularized fits above -- confirming, via a completely")
    print("  independent method, that kappa=0 (no light effect) is the principled, minimum-norm")
    print("  answer for this degenerate functional form: there is no data-driven reason to prefer")
    print("  ANY nonzero kappa here, and regularization formalizes 'prefer the simplest explanation")
    print("  consistent with the data' into a single, defensible number instead of an arbitrary pick.")

    fit_ok = True
    r_squared = r2_k0  # the data-preferred (Occam's razor) fit: kappa=0
    kappa_fit = kappa_max  # used only for the PLOT below, as the upper limit the bound permits
    logA_fit = logA_kmax

    print(f"\n--- Reporting convention for this variant ---")
    print(f"R^2 quoted below is for the kappa=0 fit (log10(A)={logA_k0:.4g}) -- the data-preferred")
    print(f"case, since kappa contributes nothing identifiable. The ratio PLOT instead shows the")
    print(f"kappa=kappa_max={kappa_max:.4g} curve (flat line, ratio={MAX_PLAUSIBLE_NPLIN_RATIO:g}x everywhere) as the most")
    print(f"OPTIMISTIC case the plausibility bound permits -- clearly NOT preferred by the data")
    print(f"over kappa=0 (ratio=1x, no effect). Both are shown; neither is a fitted discovery.")
    print(f"\n{'S':>5} {'observed A':>12} {'model P (kappa=0)':>18} {'residual':>10}")
    for S, obs, pred in zip(S_data, A_data, preds_k0):
        print(f"{S:5.2f} {obs:12.3f} {pred:18.3f} {obs - pred:10.3f}")
    print(f"\nR^2 = {r_squared:.4f}  (only 3 data points / 1 identifiable free parameter here --")
    print("2 residual degrees of freedom, so treat this R^2 as descriptive, not a strong")
    print("statistical validation either way.)")

    # Ratio plot: shows the kappa_max (upper-limit / most-optimistic) curve, which for this
    # variant is a FLAT LINE at ratio=MAX_PLAUSIBLE_NPLIN_RATIO across all S (see module
    # docstring: the ratio doesn't depend on S at fixed intensity in this functional form).
    light.coupling_efficiency = kappa_fit
    material.A_prefactor = 10 ** logA_fit
    disclaimer = (f"PREFACTOR VARIANT -- real glycine data only, NOT diamond/carbon/CO2; kappa NOT "
                   f"IDENTIFIABLE at fixed intensity (see console output) -- curve shown is the "
                   f"{MAX_PLAUSIBLE_NPLIN_RATIO:g}x upper limit the plausibility bound permits, not a fitted result")
    S_range, J0, J1 = report_and_plot_rate_vs_S(
        material, light, geom, "glycine_calibrated_ratio_prefactor.png",
    ...
    # No "small perturbation vs. dominates" check here -- that check (comparing a ln(S)
    # shift to ln(S) itself) is specific to the barrier-modifying mechanism and doesn't
    # apply when the field acts on the prefactor instead. The degeneracy demonstration
    # above already establishes the key plausibility fact for this variant.

    print("\n" + "=" * 78)
    print("PASS/FAIL CHECK: does a light-field effect persist across S=1.4-1.7,")
    print("the range where real glycine NPLIN experiments are actually conducted?")
    print("=" * 78)
    print(f"By construction, this variant's ratio is the SAME at every S (see module docstring):")
    print(f"  kappa=0:                ratio = 1x    (no effect) everywhere, including S=1.4-1.7")
    print(f"  kappa=kappa_max={kappa_max:.4g}: ratio = {MAX_PLAUSIBLE_NPLIN_RATIO:g}x  (the most the plausibility bound")
    print(f"                           permits) everywhere, including S=1.4-1.7")
    print("Verdict: DEGENERATE -- kappa is not identifiable from this dataset in this functional")
    print("form (see demonstration above), so THIS VARIANT CANNOT ANSWER whether an effect")
    print("persists in S=1.4-1.7 any differently than it would anywhere else: it has no S-shape")
    print("information to give. Whatever ratio you believe (1x to 100x) is equally consistent")
    print("with these 3 real data points once A is refit to compensate.")
    verdict = "DEGENERATE"
    ...
    print("data, with only 3 real data points. Within that limitation:")
    print("- HONEST ANSWER: this functional form (light boosts the prefactor A) CANNOT be")
    print("  tested against this dataset at all, because kappa is mathematically degenerate")
    print("  with A at a single fixed laser intensity -- any kappa from 0 up to the plausibility")
    print("  bound fits equally well (R^2 barely changes), so the data provide NO evidence for")
    print("  OR against this mechanism. This is a genuine limitation of the calibration DATA")
    print("  (single intensity, not a fluence sweep), not of the CNT model or the fitting code.")
    print("  A real test of this functional form would need nucleation-probability data taken")
    print("  at MULTIPLE laser intensities at the same S, so that the S-independent, intensity-")
    print("  dependent boost predicted by this mechanism could actually be distinguished from a")
    print("  plain rescaling of A.")
    ...
    with np.errstate(divide="ignore", invalid="ignore"):
        ratio_curve = J1 / J0  # constant = MAX_PLAUSIBLE_NPLIN_RATIO everywhere, by construction
    return {"label": "Pre-exponential-factor (kappa boosts A) -- DEGENERATE, not fitted",
            "kappa": kappa_max, "logA": logA_fit, "r_squared": r_squared, "verdict": verdict,
...
def report_and_plot_rate_vs_S(material: MaterialParams, light: LightFieldParams,
    ...
    # PRE-EXPONENTIAL-FACTOR VARIANT: the field does NOT shift ln(S) at all here (the
    # barrier is unmodified) -- report the prefactor BOOST instead, which is what this
    # variant's kappa actually controls.
    boost = prefactor_boost(u_field, material.v_m, material.T)
    print(f"\nu_field = {u_field:.3e} J/m^3 (used only to compute the prefactor boost below;")
    print(f"it is NOT a ln(S)/barrier shift in this variant -- the barrier is unmodified)")
    print(f"This boosts the prefactor A by a factor of (1 + {boost:.3e}) = {1 + boost:.3e}x "
          f"(kappa={light.coupling_efficiency:g}).")
    print(f"Because this boost does not depend on S, J(with field)/J(no field) = {1 + boost:.3e}x")
    print(f"at EVERY S, not just near the nucleation threshold -- see module docstring.")
...
def run_demo():
    ...
    print("the fitting machinery, and this variant's kappa/A degeneracy diagnostics) is")
    print("exercised end to end against real data by default.")
    print("=" * 78)

    run_calibrate_glycine()

    print("\nTo calibrate against your OWN data instead: 'python model_prefactor_variant.py")
