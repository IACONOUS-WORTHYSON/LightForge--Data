#!/usr/bin/env python3
"""
model_supersaturation_variant.py
=================================

*** ALTERNATIVE FUNCTIONAL FORM: SUPERSATURATION-SCALING VARIANT ***
This is a variant of nplin_cnt_model.py (kept as a separate, independently
rerunnable copy -- the original is not modified). Everything is identical to
the original EXCEPT how the light-field coupling kappa enters the model: the
field's contribution to ln(S) is scaled by f(S) = (S - 1), so the SAME kappa
produces a shift that GROWS with supersaturation instead of staying fixed.
See f_S_scaling() and effective_ln_S() below for the exact change, and the
module docstring's LIGHT-FIELD TERM section for why this hypothesis is being
tested (does an effect that strengthens with S survive better into the real
S=1.4-1.7 NPLIN experimental range than the original fixed-coupling model?).
THIS FUNCTIONAL FORM IS A HYPOTHESIS, NOT DERIVED FROM PHYSICS -- see the
caveat in f_S_scaling()'s docstring.
...
*** THIS VARIANT CHANGES THAT LAST LINE TO: ***
    ...
    ln(S_eff) = ln(S) + kappa * u_field * v_m / kT * f_S_scaling(S)
    f_S_scaling(S) = (S - 1)     [hypothesis-only, see f_S_scaling() docstring]

so the field's leverage over the barrier grows with supersaturation instead
of staying fixed. dG*, r*, J are then evaluated at S_eff instead of S, exactly
as in the original -- only the S-dependence of the shift itself has changed.
u_field > 0 (for eps_c > eps_s) increases ln(S_eff), which shrinks r* and
dG* -- i.e. exactly the "reduction in free energy of pre-nucleating clusters
... reducing critical nucleus size" effect described in the NPLIN literature
above, just with a different (hypothesized) S-dependence than the original.
...
Requires: numpy, scipy, matplotlib (no other dependencies).
    ...
    python model_supersaturation_variant.py demo
    ...
    python model_supersaturation_variant.py explore
    ...
    python model_supersaturation_variant.py calibrate --data yourdata.csv
    ...
    python model_supersaturation_variant.py montecarlo
    ...
    python model_supersaturation_variant.py litcheck
...
def f_S_scaling(S):
    """SUPERSATURATION-SCALING VARIANT (hypothesis, not derived from physics): the light-
    field coupling's leverage is assumed to GROW with supersaturation rather than stay
    fixed, on the hypothesis that the effect depends on the pre-nucleation cluster
    population, which itself grows with S. This is a first-pass linear form, f(S) = S - 1
    (zero at S=1 where there is no supersaturation and presumably no clusters to act on,
    growing linearly beyond that). A power-law form f(S) = (S - 1)**p is an easy extension
    -- we do not fit p here, to keep the parameter count low against only 3 real data
    points (fitting kappa, log(A), AND p from 3 points would leave zero degrees of freedom
    and produce a meaningless "perfect" fit). Compare against model_prefactor_variant.py
    (field boosts the pre-exponential factor instead) and the original nplin_cnt_model.py
    (fixed, S-independent coupling) to see how much this choice of functional form matters.
    """
    return S - 1.0
...
def effective_ln_S(S, u_field, v_m, T):
    ...
    (volume) free-energy term. SUPERSATURATION-SCALING VARIANT: the raw field-coupling
    energy density u_field (from field_energy_density(), unchanged from the original
    script) is additionally scaled by f_S_scaling(S) before being added to ln(S) -- so the
    same kappa produces a WEAKER shift at low S and a STRONGER shift at high S than the
    original fixed-coupling model, instead of a fixed shift regardless of S."""
    return np.log(S) + u_field * v_m / (K_B * T) * f_S_scaling(S)
...
def run_calibrate_glycine():
    ...
    print("*** SUPERSATURATION-SCALING VARIANT: kappa_effective = kappa * (S - 1) ***")
    print("(hypothesis-only functional form -- see f_S_scaling() docstring)")
    ...
    material = MaterialParams(name="glycine, S-scaling variant (aqueous, Javid et al. 2016 conditions)",
    ...
    print("kappa -> (linear, x(S-1)) -> ln(S_eff) -> (INVERSE SQUARE) -> dG* -> (NEG. EXPONENTIAL) -> J")
    ...
    disclaimer = (f"SUPERSATURATION-SCALING VARIANT (kappa*(S-1)) -- real glycine data only, "
                   f"NOT diamond/carbon/CO2; kappa search-bounded "
    ...
    S_range, J0, J1 = report_and_plot_rate_vs_S(
        material, light, geom, "glycine_calibrated_ratio_Sscaling.png",
    ...
    # SUPERSATURATION-SCALING VARIANT: the actual shift applied at S=1.5 includes the
    # f_S_scaling(1.5) factor -- without it this check would understate the true shift.
    rel_shift_fit = u_field_fit * material.v_m / (K_B * material.T) * f_S_scaling(1.5)
    lnS_mid = np.log(1.5)
    shift_fraction = rel_shift_fit / lnS_mid
    print("\n--- Is the fitted kappa a small perturbation, or does it dominate? (at S=1.5) ---")
    print(f"Light-induced shift to ln(S) [includes f_S_scaling(1.5)={f_S_scaling(1.5):.3g}]: "
          f"{rel_shift_fit:.4g}  vs.  ln(1.5) = {lnS_mid:.4g}")
    ...
    with np.errstate(divide="ignore", invalid="ignore"):
        ratio_curve = J1 / J0
    return {"label": "Supersaturation-scaling (kappa*(S-1))", "kappa": kappa_fit,
...
def run_demo():
    ...
    print("\nTo calibrate against your OWN data instead: 'python model_supersaturation_variant.py")
